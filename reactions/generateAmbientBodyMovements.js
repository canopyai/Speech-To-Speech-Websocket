export function generateAmbientBodyMovements({ duration }) {
    ...
    //   return [];
    // }
    const baseScalar = 0.1 + Math.random() * 0.1;
    ...
    const randomDeltas = [Math.floor(Math.random() * 2), Math.floor(Math.random() * 2), Math.floor(Math.random() * 2)];
    for (let step = 0; step < numSteps; step++) {
      ...
      frequencies.forEach((freq, index) => {
        ...
        targets[index * 2 + randomDeltas[index]] = value * envelope;
      });
